...
class TrendAttribution(object):
	...
	def Calculate_Attribution(self):
		...
		Rn_ave = self.AVE['Rn_ave'] /lhv_ave
		...
		PD_ea = ((- coeff_hq * gamma_ave * 2.6 * (1 + 0.536 * wind_ave)) / (delta_ave + coeff_hq * gamma_ave)) * trend_Eact
		...
		PDA_Ddelta = (- coeff_hq * gamma_ave * vpd_ave * 2.6 * (1 + 0.536 * wind_ave)) / ((delta_ave + coeff_hq * gamma_ave) ** 2) * trend_Delta
		PDA_Des = (coeff_hq * gamma_ave * 2.6 * (1 + 0.536 * wind_ave)) / ((delta_ave + coeff_hq * gamma_ave)) * trend_Esat
